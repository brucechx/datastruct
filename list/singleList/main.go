...
func (l *LinkList) reverse(){
	...
	var pre, cur, tmp *Node // tmp 中介节点 作为临时节点
	pre = l.head  // 前一个节点
	cur = pre.next // 从第二个节点,当前节点开始反转
	for cur != nil{
		tmp = cur.next // 提取出当前节点的下一个节点，保存到临时节点中
		cur.next = pre  // 当前节点的前一个节点作为当前节点的下一个节点，刚好实现反转
		pre = cur  // 此时当前节点则为下一次反转的当前节点的前一个节点
		cur = tmp  // 当前节点的下一个节点，此时为下一次反转的当前节点
	}
	l.head.next = nil  // ？
